...
<Tabs items={['Using Homebrew', 'Using Go']}>
  ...
  <Tab>
```bash copy
go install -tags aws github.com/risor-io/risor/cmd/risor@VERSION
```

For example:

```bash copy
go install -tags aws github.com/risor-io/risor/cmd/risor@v0.12.0
```

The list of release versions is available [on Github](https://github.com/risor-io/risor/releases).

Omit `-tags aws` if you want a smaller binary size.
